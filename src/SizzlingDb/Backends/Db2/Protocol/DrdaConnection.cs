...
using System.Text;
using SizzlingDb.Backends.Db2.Sql;
...
internal sealed class DrdaConnection
{
    ...
    public DrdaConnection(Stream stream, SizzlingDbConfig config, TraceLogger log)
    {
        _config = config;
        _mapper = new StatementMapper(config);
        Db2BackendConfig db2 = config.RequireDb2();
        _littleData = db2.LittleEndianData;
        _encoder = new ResultEncoder(_littleData, db2.ProductId);
        _log = log;
    ...
    private void RejectUnauthenticated(CommandUnit unit)
    {
        ...
        if (unit.CodePoint == CodePoints.ACCRDB)
        {
            ...
            d.ScalarString(CodePoints.RDBNAM, PadRdbnam(_config.RequireDb2().Database));
            Reply(unit, d.End().ToArray());
        }
        else
        {
            ReplyObject(unit, _encoder.BuildSqlcardError(-1542, "42505",
                "SIZZLINGDB: connection is not authenticated"));
        }
    }
    ...
    private void HandleExcsat(CommandUnit unit)
    {
        ...
        d.ScalarString(CodePoints.EXTNAM, _config.RequireDb2().ServerName);
        ...
        d.ScalarString(CodePoints.SRVCLSNM, _config.RequireDb2().ServerClassName);
        d.ScalarString(CodePoints.SRVNAM, _config.RequireDb2().ServerName);
        d.ScalarString(CodePoints.SRVRLSLV, _config.RequireDb2().ProductId);
        Reply(unit, d.End().ToArray());
    }
    ...
    private void HandleAccrdb(CommandUnit unit)
    {
        ...
        d.ScalarString(CodePoints.PRDID, _config.RequireDb2().ProductId);
        d.ScalarString(CodePoints.TYPDEFNAM, _config.RequireDb2().TypeDefName);
        ...
        d.End();
        Reply(unit, d.End().ToArray());
        _log.Command($"ACCRDB database accessed (PRDID={_config.RequireDb2().ProductId}, {_config.RequireDb2().TypeDefName}, compactSQLDARD={_useCompactSqldard})");
    }
    ...
    private void HandlePrepare(CommandUnit unit)
    {
        ...
        switch (_prepared)
        {
            ...
            case ResultSetResponse rs when returnSqlda:
                ...
                ReplyObject(unit, _encoder.BuildSqldard(rs.Columns, _config.RequireDb2().Database, _useCompactSqldard));
                break;
            default:
    ...
    private void HandleOpenQuery(CommandUnit unit)
    {
        ...
        if (resp is ResultSetResponse rs)
        {
            ...
            ReplyObject(unit, _encoder.BuildSqlcardEndOfData(rs.Rows.Count, _config.RequireDb2().Database));
        }
        ...
        else
        {
            ...
            ReplyObject(unit, _encoder.BuildSqlcardError(-204, "42704",
                "SIZZLINGDB: statement does not produce a result set"));
        }
    }

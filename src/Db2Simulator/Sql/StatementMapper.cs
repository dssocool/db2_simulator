...
internal sealed class StatementMapper
{
    ...
    public StatementMapper(SimulatorConfig config) => _config = config;
    ...
    public StatementResponse Resolve(string sql)
    {
        ...
        return new ErrorResponse(-204, "42704",
            $"DB2SIM: no mapping configured for statement: {sql.Trim()}");
    }
    ...
    private static ResultSetResponse BuildResultSet(ResultConfig result)
    {
        ...
        {
            SimColumnType type = SimColumnTypeExtensions.Parse(c.Type);
            int length = c.Length ?? DefaultLength(type);
            int precision = c.Precision ?? (type == SimColumnType.Decimal ? 9 : 0);
            int scale = c.Scale ?? (type == SimColumnType.Decimal ? 2 : 0);
    ...
    private static int DefaultLength(SimColumnType t) => t switch
    {
        SimColumnType.Char => 1,
        SimColumnType.Varchar => 255,
    ...
    private static object? ConvertCell(JsonElement cell, SimColumnType type)
    {
        ...
        return type switch
        {
            SimColumnType.Char or SimColumnType.Varchar
                or SimColumnType.Date or SimColumnType.Time or SimColumnType.Timestamp
                => CellToString(cell),
            SimColumnType.Smallint or SimColumnType.Integer or SimColumnType.Bigint
                => CellToLong(cell),
            SimColumnType.Real or SimColumnType.Double
                => CellToDouble(cell),
            SimColumnType.Decimal
                => CellToDecimal(cell),
            _ => CellToString(cell),
        };
    }

...
{
    private static readonly Lazy<(SimulatorConfig? Config, string? Error)> Loaded = new(Load);

    public static DatabaseConnectionConfig RequireDb2()
    {
        SimulatorConfig config = Require();
    ...
    private static SimulatorConfig Require()
    {
        (SimulatorConfig? config, string? error) = Loaded.Value;
        Skip.If(config is null, $"config/config.json could not be loaded: {error}");
        return config!;
    }

    private static (SimulatorConfig?, string?) Load()
    {
        try
        {
            return (SimulatorConfig.Load(ConfigPath.Resolve()), null);
        }
        catch (Exception ex)
        {
